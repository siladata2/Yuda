...
export class AnichinCare {
  ...
  async completed(page = 1) {
    return this._filteredList(`${BASE_URL}/completed/`, page);
  }

  async ongoing(page = 1) {
    return this._filteredList(`${BASE_URL}/ongoing/`, page);
  }

  async movie(page = 1) {
    return this._filteredList(`${BASE_URL}/?type=movie`, page);
  }

  async _filteredList(base, page = 1) {
    const url = base + (base.includes('?') ? '&' : (base.endsWith('/') ? '' : '/')) + (page > 1 ? (base.includes('?') ? `page=${page}/` : `page/${page}/`) : '');
