...
// ============================================
// HTML QR CODE GENERATOR - FIXED & WORKING
// ============================================
const qrHtml = `
<!DOCTYPE html>
<html lang="en">
...
// MIT License - full working implementation
// ============================================
var qrcode = (function() {
  'use strict';
...
function renderQRToCanvas(text, canvas, size) {
  ...
  var typeNumber = 1;
  var ecl = qrcode.QRErrorCorrectLevel.M;
  for (var t = 1; t <= 40; t++) {
    try {
      var test = new qrcode.QRCode(t, ecl);
      test.addData(text);
      test.make();
      typeNumber = t;
      break;
    } catch (e) {
      if (t === 40) throw new Error('Text too long for QR code');
    }
  }

  var qr = new qrcode.QRCode(typeNumber, ecl);
  qr.addData(text);
  qr.make();
  ...
  var offset = Math.floor((size - actualSize) / 2);
...
(function() {
  ...
  function onGenerate() {
    ...
    try {
      renderQRToCanvas(text, qrCanvas, 440);
