const motoHtml = `
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
<style>
:root{--card-2:#2a3942;--ink:#e9edef;--muted:#8696a0;--accent:#00a884;--line:#2a3942;--cell-bg:#111b21;
--sys:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;}
*{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent;user-select:none;}
html,body{background:transparent;color:var(--ink);font-family:var(--sys);min-height:100vh;overflow:hidden;touch-action:none;}
.stage{min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px 16px;}
.card{width:100%;max-width:360px;}
.header{display:flex;align-items:baseline;justify-content:space-between;margin-bottom:14px;padding-bottom:12px;border-bottom:1px solid var(--line);}
.header__title{font-size:17px;font-weight:600;}
.header__sub{font-size:12px;color:var(--muted);}
.stats{display:flex;justify-content:space-between;font-size:12px;color:var(--muted);margin-bottom:14px;}
.stats b{color:var(--ink);font-weight:600;margin-left:4px;}
.wrap{position:relative;background:#0b141a;border:1px solid var(--line);border-radius:12px;overflow:hidden;box-shadow:0 10px 30px -10px rgba(0,0,0,.6);}
canvas{display:block;width:100%;}
.overlay{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:10px;background:rgba(11,20,26,0.78);}
.overlay h2{font-size:19px;text-align:center;padding:0 16px;}
.overlay button{background:var(--accent);border:none;border-radius:8px;color:#0b141a;font-weight:700;font-family:inherit;padding:10px 22px;font-size:14px;cursor:pointer;}
.controls{display:flex;justify-content:space-between;margin-top:12px;gap:10px;}
.ctrl{flex:1;background:var(--card-2);border:1px solid var(--line);border-radius:10px;padding:14px 0;text-align:center;font-size:20px;color:var(--ink);cursor:pointer;}
.ctrl:active{background:var(--accent);}
.hint{font-size:11px;color:var(--muted);text-align:center;margin-top:8px;}
</style>
</head>
<body>
<main class="stage">
  <div class="card">
    <div class="header">
      <div class="header__title">SILA Moto Night</div>
      <div class="header__sub">Collect coins</div>
    </div>
    <div class="stats"><span>Coins<b id="score">0</b></span><span>Best<b id="best">0</b></span><span>Speed<b id="spd">1x</b></span></div>
    <div class="wrap" id="wrap">
      <canvas id="canvas" width="320" height="440"></canvas>
      <div class="overlay" id="overlay">
        <h2 id="ov-title">SILA Moto Night</h2>
        <button id="ov-btn">Rev Up</button>
      </div>
    </div>
    <div class="controls">
      <div class="ctrl" id="left">◀</div>
      <div class="ctrl" id="right">▶</div>
    </div>
    <div class="hint">Swipe or tap arrows — grab 🪙, dodge barrels</div>
  </div>
</main>
<script>
(function(){
const canvas=document.getElementById('canvas');
const ctx=canvas.getContext('2d');
const W=canvas.width,H=canvas.height;
const overlay=document.getElementById('overlay');
const ovTitle=document.getElementById('ov-title');
const ovBtn=document.getElementById('ov-btn');
const roadL=W*0.18,roadR=W*0.82,roadW=roadR-roadL,lanes=3,laneW=roadW/lanes;
let player,obs,coins,buildings,score,best=0,running=false,frame=0,speed=5.2,dashOffset=0,lean=0;

function laneX(i){return roadL+laneW*i+laneW/2;}
function initBuildings(){
  buildings=[];
  for(let i=0;i<10;i++){
    buildings.push({x:i*36-20,side:'l',h:40+Math.random()*90,w:26+Math.random()*10});
    buildings.push({x:i*36-20,side:'r',h:40+Math.random()*90,w:26+Math.random()*10});
  }
}
function reset(){
  player={lane:1,targetLane:1,x:laneX(1),y:H-90,w:26,h:50};
  obs=[];coins=[];score=0;frame=0;speed=5.2;lean=0;
  initBuildings();
  document.getElementById('score').textContent='0';
  document.getElementById('spd').textContent='1x';
}
function start(){reset();running=true;overlay.style.display='none';loop();}
function gameOver(){
  running=false;
  if(score>best) best=score;
  document.getElementById('best').textContent=best;
  ovTitle.textContent='Wiped out! Coins '+score;
  ovBtn.textContent='Try again';
  overlay.style.display='flex';
}
function moveLane(dir){
  if(!running) return;
  player.targetLane=Math.max(0,Math.min(lanes-1,player.targetLane+dir));
  lean=dir*0.35;
}
function spawnObs(){
  const lane=Math.floor(Math.random()*lanes);
  obs.push({lane,x:laneX(lane),y:-60,r:16});
}
function spawnCoin(){
  const lane=Math.floor(Math.random()*lanes);
  coins.push({lane,x:laneX(lane),y:-40,r:10,taken:false,spin:0});
}
function update(){
  frame++;
  document.getElementById('spd').textContent=(speed/5.2).toFixed(1)+'x';
  speed=5.2+Math.min(6,score*0.015);
  dashOffset=(dashOffset+speed)%40;
  player.x+=(laneX(player.targetLane)-player.x)*0.25;
  lean*=0.9;
  buildings.forEach(b=>{b.x-=speed*0.6; if(b.x<-40) b.x+=360;});
  if(frame%Math.max(35,60-Math.floor(score/3))===0) spawnObs();
  if(frame%50===0) spawnCoin();
  obs.forEach(o=>o.y+=speed);
  obs=obs.filter(o=>o.y<H+40);
  coins.forEach(c=>{c.y+=speed;c.spin+=0.2;});
  coins=coins.filter(c=>c.y<H+30&&!c.taken);
  for(const o of obs){
    if(Math.abs(o.x-player.x)<o.r+12&&Math.abs(o.y-player.y)<o.r+18){
      return gameOver();
    }
  }
  for(const c of coins){
    if(!c.taken&&Math.abs(c.x-player.x)<24&&Math.abs(c.y-player.y)<28){
      c.taken=true;score+=1;document.getElementById('score').textContent=score;
    }
  }
}
function drawSkyline(){
  ctx.fillStyle='#0d1520';
  buildings.forEach(b=>{
    const x=b.side==='l'?b.x*0.3:roadR+18+ (360-b.x)*0.02;
    const y=H*0.4-b.h;
    const bx=b.side==='l'?b.x*0.25:W-b.x*0.25-b.w;
    ctx.fillRect(bx,H*0.42-b.h,b.w,b.h);
    ctx.fillStyle='#f2c265';
    for(let wy=H*0.42-b.h+8;wy<H*0.4-4;wy+=14){
      for(let wx=bx+4;wx<bx+b.w-6;wx+=10){
        if(Math.random()<0.4) ctx.fillRect(wx,wy,4,6);
      }
    }
    ctx.fillStyle='#0d1520';
  });
}
function roundRect(x,y,w,h,r){
  ctx.beginPath();
  ctx.moveTo(x+r,y);ctx.arcTo(x+w,y,x+w,y+h,r);ctx.arcTo(x+w,y+h,x,y+h,r);
  ctx.arcTo(x,y+h,x,y,r);ctx.arcTo(x,y,x+w,y,r);ctx.closePath();
}
function drawBike(){
  ctx.save();
  ctx.translate(player.x,player.y);
  ctx.rotate(lean);
  ctx.shadowColor='rgba(0,0,0,0.5)';ctx.shadowBlur=8;ctx.shadowOffsetY=4;
  ctx.fillStyle='#1a1f26';
  ctx.beginPath();ctx.ellipse(0,20,7,8,0,0,Math.PI*2);ctx.fill();
  ctx.beginPath();ctx.ellipse(0,-20,7,8,0,0,Math.PI*2);ctx.fill();
  ctx.shadowBlur=0;
  const grad=ctx.createLinearGradient(-8,-18,8,18);
  grad.addColorStop(0,'#00c2a0');grad.addColorStop(1,'#00453a');
  ctx.fillStyle=grad;
  roundRect(-7,-18,14,36,6);ctx.fill();
  ctx.fillStyle='#fff6c8';
  ctx.shadowColor='#fff6c8';ctx.shadowBlur=12;
  ctx.beginPath();ctx.ellipse(0,-24,3,4,0,0,Math.PI*2);ctx.fill();
  ctx.shadowBlur=0;
  ctx.restore();
}
function draw(){
  const sky=ctx.createLinearGradient(0,0,0,H);
  sky.addColorStop(0,'#05070f');
  sky.addColorStop(0.42,'#131b33');
  sky.addColorStop(1,'#111b21');
  ctx.fillStyle=sky;ctx.fillRect(0,0,W,H);
  drawSkyline();
  ctx.fillStyle='#1c2228';
  ctx.fillRect(0,H*0.4,W,H*0.6);
  const road=ctx.createLinearGradient(0,0,0,H);
  road.addColorStop(0,'#232a30');road.addColorStop(1,'#171e23');
  ctx.fillStyle=road;
  ctx.fillRect(roadL,0,roadW,H);
  ctx.strokeStyle='#f2c265';ctx.lineWidth=3;
  ctx.beginPath();ctx.moveTo(roadL,0);ctx.lineTo(roadL,H);ctx.stroke();
  ctx.beginPath();ctx.moveTo(roadR,0);ctx.lineTo(roadR,H);ctx.stroke();
  ctx.strokeStyle='rgba(233,237,239,0.5)';ctx.lineWidth=3;
  ctx.setLineDash([16,16]);ctx.lineDashOffset=-dashOffset;
  for(let i=1;i<lanes;i++){
    const x=roadL+laneW*i;
    ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,H);ctx.stroke();
  }
  ctx.setLineDash([]);
  ctx.fillStyle='#3a2a1a';
  obs.forEach(o=>{
    ctx.save();
    ctx.shadowColor='rgba(0,0,0,0.5)';ctx.shadowBlur=6;ctx.shadowOffsetY=3;
    const grad=ctx.createLinearGradient(o.x-o.r,o.y-o.r,o.x+o.r,o.y+o.r);
    grad.addColorStop(0,'#8b5e34');grad.addColorStop(1,'#4a3018');
    ctx.fillStyle=grad;
    roundRect(o.x-o.r,o.y-o.r*1.3,o.r*2,o.r*2.6,6);ctx.fill();
    ctx.restore();
  });
  coins.forEach(c=>{
    const sc=Math.abs(Math.cos(c.spin));
    ctx.save();
    ctx.translate(c.x,c.y);
    ctx.scale(Math.max(0.2,sc),1);
    ctx.fillStyle='#f2c265';
    ctx.shadowColor='#f2c265';ctx.shadowBlur=8;
    ctx.beginPath();ctx.arc(0,0,c.r,0,Math.PI*2);ctx.fill();
    ctx.restore();
  });
  drawBike();
}
function loop(){
  if(!running) return;
  update();
  if(running){draw();requestAnimationFrame(loop);}
}
document.getElementById('left').addEventListener('click',()=>moveLane(-1));
document.getElementById('right').addEventListener('click',()=>moveLane(1));
document.addEventListener('keydown',(e)=>{
  if(e.key==='ArrowLeft') moveLane(-1);
  if(e.key==='ArrowRight') moveLane(1);
});
let sx=0;
document.getElementById('wrap').addEventListener('touchstart',(e)=>{sx=e.touches[0].clientX;});
document.getElementById('wrap').addEventListener('touchend',(e)=>{
  const dx=e.changedTouches[0].clientX-sx;
  if(Math.abs(dx)>30) moveLane(dx>0?1:-1);
});
ovBtn.addEventListener('click',start);
initBuildings();
draw();
})();
</script>
</body>
</html>
`;

export default {
  name: 'moto',
  alias: ['motoracer', 'pikipiki'],
  description: 'Play Moto Night Racer — collect coins, dodge barrels in WhatsApp',
  category: 'games',
  ownerOnly: false,

  async execute(sock, msg, args, prefix, options) {
    const sender = msg.key.remoteJid;
    try {
      const responseId = 'sila-moto-' + Date.now();
      const content = {
        messageContextInfo: {
          deviceListMetadata: {},
          deviceListMetadataVersion: 2,
          messageSecret: "0cCzjnQ5ERoqM2QrQ7KjmMfxsyeWYu+61/chr2wioyE=",
          botMetadata: { messageDisclaimerText: "", botResponseId: responseId }
        },
        botForwardedMessage: {
          message: {
            richResponseMessage: {
              messageType: 1,
              submessages: [{ messageType: 2, messageText: "🏍️ Moto Night" }],
              unifiedResponse: {
                data: Buffer.from(JSON.stringify({
                  "response_id": responseId,
                  "sections": [{
                    "view_model": {
                      "primitive": {
                        "__typename": "GenAIaeacdsnwHtmlPrimitive",
                        "payload": motoHtml,
                        "trusted_sources": ["sila-tech"]
                      },
                      "__typename": "GenAISingleLayoutViewModel"
                    }
                  }]
                })).toString('base64')
              },
              contextInfo: {
                forwardingScore: 1,
                isForwarded: true,
                forwardedAiBotMessageInfo: { botJid: "867051314767696@bot" },
                forwardOrigin: 4
              }
            }
          }
        }
      };
      await sock.relayMessage(sender, content, {});
    } catch (error) {
      console.error('[MOTO]', error);
      await sock.sendMessage(sender, { text: `✖ Error: ${error?.message || error}` }, { quoted: msg });
    }
  }
};
